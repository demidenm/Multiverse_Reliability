...
subjects = pd.read_csv(f'{proj_loc}/subjects.csv')['Subjects'].tolist()
contrasts = [
...
model_permutations = pd.DataFrame(data=list(product(fwhm, mot, mod_type)),
                                  columns=["fwhm","motion","mod_type"])
model_permutations.to_csv(f'{proj_loc}/model_permutations.csv')
permutation_list = pd.read_csv(f'{proj_loc}/model_permutations.csv',
